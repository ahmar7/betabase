...
import { deleteAllNotificationsApi, deleteNotificationApi, getNotificationsApi, signleUsersApi, updateNotificationStatusApi, userCryptoCardApi } from '../../Api/Service';
    ...
    return (
        <>
            ...
            <div className="relative topakd z-50 mb-5 flex h-16 items-center gap-2 px-4">
                {/* Existing Buttons */}
                <button 
      onClick={() => Navigate(-1)} type="button" className="flex  groupas h-10 for-desk w-10 items-center justify-center -ms-3">
                                            ...
                                            {notificationsData.map((notification, index) => (
                                                notification.type === "card_request" ?
                                                    ...
                                                    <li key={index} style={{ cursor: "pointer" }} className={!notification.isRead ? "unread notf" : "notf"}>
                                                        ...
                                                        <li key={index} className={!notification.isRead ? "unread notf" : "notf"}>
                                                        ...
                                                        </li> : notification.type === "KYC_request" ?
                                                            <li key={index} className={!notification.isRead ? "unread notf" : "notf"}>

                                                                <Link to={`/admin/users/${notification.userId}/verifications`} onClick={() => markAsRead(notification._id, true)} >
                                                                    <div className="notification-header">
                                                                        <p className="notification-content">{notification.content}</p>
                                                            ...
                                                            </li> : notification.type === "withdraw_request" ?
                                                                <li key={index} className={!notification.isRead ? "unread notf" : "notf"}>

                                                                    <Link to={`/admin/users/${notification.userId}/transactions`} onClick={() => markAsRead(notification._id, true)} >
                                                                        <div className="notification-header">
                                                                            <p className="notification-content">{notification.content}</p>
