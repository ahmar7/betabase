...
const UserSideBar = (props) => {
  const [isTokenManagementUpdate, setisTokenManagementUpdate] = useState(false);
  ...
  const getSignleUser = async () => {
    try {
      ...
      if (signleUser.success) {
        if (signleUser.signleUser.role === 'subadmin') {
          setsubAdminPermissions(signleUser.signleUser?.permissions)
        }
        if (signleUser.signleUser?.adminPermissions?.isTokenManagement === false && signleUser.signleUser.role === 'admin') {
          setisTokenManagementUpdate(false)
          return
        } else {
          setisTokenManagementUpdate(true)
        }
      } else {
      }
    } catch (error) {
      ...
      <div className="col-span-12 sm:col-span-4">
        <div className="mt-8 max-w-[240px]">
          <ul className="space-y-1 font-sans text-sm">
            {subAdminPermissions?.editUserProfile === false ?
              "" :
            ...
            {subAdminPermissions?.addTransaction === false ?
              "" :
              <li>
                <NavLink
            ...
            {isTokenManagementUpdate &&

              <li>
                <NavLink
                  to={`/admin/users/${props.userid}/tokens`}
                  className="text-muted-400 datas hover:text-muted-600 dark:hover:text-muted-200 hover:bg-muted-50 dark:hover:bg-muted-700/50 flex items-center gap-2 rounded-lg p-3 transition-colors duration-300"
                >
                  <svg
                    data-v-cd102a71
