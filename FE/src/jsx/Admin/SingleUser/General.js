...
const General = () => {
  ...
  const getActiveSignleUser = async (user) => {
    try {
      ...
      if (signleUser.success) {
        if (user.role != "user" && signleUser.signleUser.adminPermissions?.isSubManagement === false && signleUser.signleUser.role === "admin") {
          Navigate("/admin/dashboard")
        }
        if (signleUser.signleUser.role === "subadmin" && signleUser.signleUser.permissions.editUserProfile == false) {
