...
function Login(props) {
	...
	const onLogin = async (e) => {
		...
		try {
			const { success, msg, token, user, link } = await loginApi({ email, password });


			if (success && !link) {
				// Save to localStorage yourself (optional if using signIn)
				localStorage.setItem("authToken", token);
				localStorage.setItem("authUser", JSON.stringify(user));

				// AuthProvider’s built-in signIn also writes to localStorage
				const ok = signIn({
					token: token.token,
					expiresIn: 60 * 60 * 24, // e.g., 1 day
					tokenType: "Bearer",
					authState: { user },
				});
				...
				if (ok) {
					toast.dismiss();
					toast.success(msg);
					window.location.href = user.role === "user" ? "/dashboard" : "/admin/dashboard";
